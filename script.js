class Calculator {
    ...
    clear() {
        ...
        this.isSpecialResult = false;
    }

    delete() {
        if (this.currentOperand === '0' || this.isSpecialResult) return;
        this.currentOperand = this.currentOperand.toString().slice(0, -1);
        if (this.currentOperand === '') this.currentOperand = '0';
    }

    appendNumber(number) {
        if (this.isSpecialResult) {
            this.clear();
        }
    ...
    chooseOperation(operation) {
        if (this.isSpecialResult) return;
    ...
    compute() {
        ...
        switch (this.operation) {
            ...
            case '×':
                ...
                break;
            case '÷':
                // MODIFICACIÓN PARA DIVISIÓN ENTRE 0
                if (current === 0 && prev === 0) {
                    computation = "ERROR";  // 0 ÷ 0 = ERROR
                    this.isSpecialResult = true;
                } else if (current === 0) {
                    computation = "∞";      // Cualquier número ÷ 0 = ∞
                    this.isSpecialResult = true;
                } else {
                    computation = prev / current;  // División normal
                    this.isSpecialResult = false;
                }
                break;
            default:
        ...
        // GUARDAR EN HISTORIAL - INCLUSO LOS RESULTADOS ESPECIALES
